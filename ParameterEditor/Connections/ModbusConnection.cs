...
namespace ParameterEditor.Connections
{
    public class ModbusConnection : IDisposable
    {
        ...
        public ModbusConnection(int _slvaId)
        {
            try
            {
                if (!Main_ModbusClient.Connected)
                {
                    SlaveId = (byte)_slvaId;
                    ...
                    foreach (string _port in _ports)
                    {
                        ...
                        Main_ModbusClient.ConnectionTimeout = 1000;
                        Main_ModbusClient.UnitIdentifier = SlaveId;
        ...
        public bool SetRestoreFactorySettings()
        {
            try
            {
                if (Main_ModbusClient.Connected)
                {
                    ...
                    {
                        DialogResult _ans = _msgBox.Show("W@shWis elektronik kartını fabrika ayarlarını geri döndürmek istediğinize emin misiniz?", "FABRİKA AYARLARI", MsgBox.Buttons.YesNo, MsgBox.Icons.Question);
                        if (_ans == DialogResult.Yes)
                        {
                            Main_ModbusClient.WriteSingleRegister(6, 1);
                            System.Threading.Thread.Sleep(100);
                            if (Main_ModbusClient.ReadHoldingRegisters(6, 1)[0] == 0)
                            {
                                return true;
                            }
                            else
